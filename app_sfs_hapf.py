...
separation_distance = 2  # Minimum separation distance between drones and obstacles
...
# Stochastic Fractal Search (SFS) Algorithm Implementation
...
def sfs(start, goal, obstacles, space_size, paths_taken, max_iter=1000):
...
# Calculate paths using SFS and HAPF
def calculate_paths():
    ...
    for i in range(num_drones):
        ...
        print(f"Calculating path for drone {i + 1} from {start} to {goal}")  # Corrected drone ID
        path = sfs(start, goal, hexahedrons, space_size, paths)
        attempts = 0
        while not path and attempts < 5:  # Retry up to 5 times if path is empty
            print(f"Retrying path for drone {i + 1} (Attempt {attempts + 1})")  # Corrected drone ID
            path = sfs(start, goal, hexahedrons, space_size, paths)
            attempts += 1
        if not path:
            print(f"Drone {i + 1} could not find a path.")  # Corrected drone ID
        else:
            path = adjust_path_with_hapf(path, hexahedrons)
            paths.append(path)
            print(f"Path for drone {i + 1} found with {len(path)} waypoints.")  # Corrected drone ID
