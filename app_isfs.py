...
def initialize_population(pop_size, num_drones):
    ...
    for _ in range(pop_size):
        ...
        paths = [adjust_path_with_hapf(path, hexahedrons) for path in paths]  # Adjust paths to avoid obstacles
...
def mutate(individual):
    ...
    individual[0][idx] = adjust_path_with_hapf(individual[0][idx], hexahedrons)  # Adjust path to avoid obstacles
...
# Communication: Share planned paths among drones
def share_paths(paths):
    shared_paths = []
    for path in paths:
        shared_paths.append(np.array(path))
    return shared_paths

# Trajectory Adjustment: Adjust paths to avoid conflicts
def adjust_paths(paths, shared_paths):
    adjusted_paths = []
    for i, path in enumerate(paths):
        for j, shared_path in enumerate(shared_paths):
            if i != j:
                for t in range(min(len(path), len(shared_path))):
                    if np.linalg.norm(np.array(path[t]) - np.array(shared_path[t])) < separation_distance:
                        # Adjust path to avoid conflict
                        path[t] = (path[t][0], path[t][1], path[t][2] + height_offset)
        adjusted_paths.append(path)
    return adjusted_paths

# Calculate paths using the optimization algorithm and Robust MADER for avoidance
def calculate_paths():
    global paths, velocities, ETDs
    best_individual = optimize_routes()
    paths, velocities, ETDs = best_individual

    # Share paths among drones
    shared_paths = share_paths(paths)

    # Adjust paths for collision avoidance
    paths = adjust_paths(paths, shared_paths)
